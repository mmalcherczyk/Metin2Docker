...
int CShopEx::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	{
		SPDLOG_ERROR("ShopEx::Buy : invalid position {} : {}", pos, ch->GetName());
		return SHOP_SUBHEADER_GC_INVALID_POS;
	}

	SPDLOG_DEBUG("ShopEx::Buy : name {} pos {}", ch->GetName(), pos);
	...
	case SHOP_COIN_TYPE_GOLD:
		...
		{
			SPDLOG_DEBUG("ShopEx::Buy : Not enough money : {} has {}, price {}", ch->GetName(), ch->GetGold(), dwPrice);
			return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY;
		}
		break;
	case SHOP_COIN_TYPE_SECONDARY_COIN:
		{
			...
			if (count < dwPrice)
			{
				SPDLOG_DEBUG("ShopEx::Buy : Not enough myeongdojun : {} has {}, price {}", ch->GetName(), count, dwPrice);
	...
	if (iEmptyPos < 0)
	{
		SPDLOG_DEBUG("ShopEx::Buy : Inventory full : {} size {}", ch->GetName(), item->GetSize());
