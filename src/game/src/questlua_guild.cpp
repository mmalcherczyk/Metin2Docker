...
namespace quest
{
	...
	int guild_war_bet(lua_State * L)
	{
		...
		SPDLOG_DEBUG("GUILD_WAR_BET: {} login {} war_id {} guild {} gold {}",
	...
	int guild_get_reserve_war_table(lua_State * L)
	{
		...
		SPDLOG_DEBUG("con.size(): {}", con.size());
		...
		while (it != con.end())
		{
			...
			SPDLOG_DEBUG("con.size(): {} {} {} handi {}", p->dwID, p->dwGuildFrom, p->dwGuildTo, p->lHandicap);
