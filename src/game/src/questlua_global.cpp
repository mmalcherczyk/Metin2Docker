...
namespace quest
{
	...
	int _set_skin(lua_State* L) // Quest UI style
	{
		...
		{
			quest_err("QUEST wrong skin index");
		}
	...
	int _set_server_timer(lua_State* L)
	{
		...
		{
			quest_err("QUEST set_server_timer argument count wrong.");
			return 0;
		}
	...
	int _set_named_loop_timer(lua_State* L)
	{
		...
		if (n != 2 || !lua_isnumber(L, -1) || !lua_isstring(L, -2))
			quest_err("QUEST set_timer argument count wrong.");
		else
		{
	...
	int _set_timer(lua_State* L)
	{
		if (lua_gettop(L) != 1 || !lua_isnumber(L, -1))
			quest_err("QUEST invalid argument.");
		else
		{
			double t = lua_tonumber(L, -1);
	...
	{
        SPDLOG_TRACE("_raw_script : {} ", lua_tostring(L,-1));
		if (lua_isstring(L, -1))
			CQuestManager::Instance().AddScript(lua_tostring(L,-1));
		else
			quest_err("QUEST wrong argument: questname: %s", CQuestManager::instance().GetCurrentQuestName().c_str());
	...
	int _syslog(lua_State* L)
	{
		...
		SPDLOG_DEBUG("QUEST: quest: {} player: {} : {}", pc->GetCurrentQuestName().c_str(), ch->GetName(), lua_tostring(L, 2));
	...
	int _syserr(lua_State* L)
	{
		...
		quest_err("QUEST: quest: %s player: %s : %s", pc->GetCurrentQuestName().c_str(), ch->GetName(), lua_tostring(L, 1));
	...
	int _give_char_privilege(lua_State* L)
	{
		...
		{
			quest_err("PRIV_MANAGER: _give_char_privilege: wrong empire priv type(%u)", type);
			return 0;
		}
	...
	int _give_empire_privilege(lua_State* L)
	{
		...
		if (MAX_PRIV_NUM <= type)
		{
			quest_err("PRIV_MANAGER: _give_empire_privilege: wrong empire priv type(%u)", type);
			return 0;
		}

		if (ch)
			SPDLOG_DEBUG("_give_empire_privileage(empire={}, type={}, value={}, time={}), by quest, {}",
					empire, type, value, time, ch->GetName());
		else
			SPDLOG_DEBUG("_give_empire_privileage(empire={}, type={}, value={}, time={}), by quest, NULL",
	...
	int _give_guild_privilege(lua_State* L)
	{
		...
		if (MAX_PRIV_NUM <= type)
		{
			quest_err("PRIV_MANAGER: _give_guild_privilege: wrong empire priv type(%u)", type);
			return 0;
		}

		SPDLOG_DEBUG("_give_guild_privileage(empire={}, type={}, value={}, time={})",
	...
	{
		if ( !lua_isstring( L, 1 ) ) {
			quest_err( "_get_guildid_byname() - invalud argument" );
			lua_pushnumber( L, 0 );
			return 1;
		}
	...
	int _find_pc_cond(lua_State* L)
	{
        ...
        SPDLOG_TRACE("find_pc_cond map={}, job={}, level={}~{}",
                ch->GetMapIndex(),
                uiJobFlag,
                iMinLev, iMaxLev);
	...
	int _find_npc_by_vnum(lua_State* L)
	{
		...
		//quest_err("not find(race=%d)", race);
	...
	int _set_quest_state(lua_State* L)
	{
		...
		if (L!=pqs->co) 
		{
			...
			SPDLOG_DEBUG("running thread != current thread???");
			return -1;
		}
		if (pPC)
		{
            ...
            SPDLOG_TRACE("set_state {} {} ", stQuestName.c_str(), stStateName.c_str() );
	...
	int _get_quest_state(lua_State* L)
	{
		...
		if (pPC)
		{
            ...
            SPDLOG_TRACE("Get_quest_state name {} value {}", stQuestName.c_str(), nRet );
		}
		else
		{
            SPDLOG_TRACE("PC == 0");
		...
		void operator()(LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC())
				{
					...
					if ( bEmpire == 0 )
					{
						quest_err( "Unkonwn Empire %s %d ", ch->GetName(), ch->GetPlayerID() );
						return;
					}
	...
	EVENTFUNC(warp_all_to_village_event)
	{
		...
		if ( info == NULL )
		{
			quest_err( "warp_all_to_village_event> <Factor> Null pointer" );
			return 0;
		}
	...
	int _add_ox_quiz(lua_State* L)
	{
		...
		{
			SPDLOG_ERROR("OXEVENT : Cannot add quiz. {} {} {}", level, quiz, answer);
		}
	...
	EVENTFUNC(warp_all_to_map_my_empire_event)
	{
		...
		if ( info == NULL )
		{
			quest_err( "warp_all_to_map_my_empire_event> <Factor> Null pointer" );
			return 0;
		}
	...
	int _spawn_mob(lua_State* L)
	{
		...
		if( NULL != pMonster )
		{
			...
			SPDLOG_DEBUG("QUEST Spawn Monstster: VNUM({}) COUNT({}) isAggresive({})", dwVnum, SpawnCount, isAggresive);
		}
	...
	int _warp_all_in_area_to_area( lua_State* L )
	{
		...
		if (0 == mapIndex)
		{
			quest_err("_warp_all_in_area_to_area: cannot get a map index with (%u, %u)", from_x1, from_y1);
			lua_pushnumber(L, 0);
			return 1;
		}
		...
		if (NULL != pSectree)
		{
			...
			SPDLOG_DEBUG("_warp_all_in_area_to_area: {} character warp", func.warpCount);
			return 1;
		}
		else
		{
			lua_pushnumber(L, 0);
			quest_err("_warp_all_in_area_to_area: no sectree");
			return 1;
		}
	}
