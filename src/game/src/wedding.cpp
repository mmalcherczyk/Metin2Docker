...
namespace marriage
{
	...
	EVENTFUNC(wedding_end_event)
	{
		...
		if ( info == NULL )
		{
			SPDLOG_ERROR("wedding_end_event> <Factor> Null pointer" );
			return 0;
		}
	...
	struct FDestroyEveryone
	{
		void operator() (LPCHARACTER ch)
		{
			SPDLOG_DEBUG("WeddingMap::DestroyAll: {}", ch->GetName());
	...
	{
		SPDLOG_DEBUG("WeddingMap::DestroyAll: m_set_pkChr size {}", m_set_pkChr.size());
	...
	DWORD WeddingManager::__CreateWeddingMap(DWORD dwPID1, DWORD dwPID2)
	{
		...
		if (!dwMapIndex)
		{
			SPDLOG_ERROR("CreateWeddingMap(pid1={}, pid2={}) / CreatePrivateMap({}) FAILED", dwPID1, dwPID2, WEDDING_MAP_INDEX);
			return 0;
		}
		...
		{
			SPDLOG_ERROR("CreateWeddingMap(pid1={}, pid2={}) / regen_do(fileName={}, mapIndex={}, basePos=({}, {})) FAILED", dwPID1, dwPID2, st_weddingMapRegenFileName.c_str(), dwMapIndex, pkSectreeMap->m_setting.iBaseX, pkSectreeMap->m_setting.iBaseY);
		}
		else
		{
			SPDLOG_DEBUG("CreateWeddingMap(pid1={}, pid2={}) / regen_do(fileName={}, mapIndex={}, basePos=({}, {})) ok", dwPID1, dwPID2, st_weddingMapRegenFileName.c_str(), dwMapIndex, pkSectreeMap->m_setting.iBaseX, pkSectreeMap->m_setting.iBaseY);
		}
	...
	{
		SPDLOG_DEBUG("DestroyWeddingMap(index={})", pMap->GetMapIndex());
		pMap->DestroyAll();
