	...
	void operator()(LPENTITY entity) {
		...
		if (entity->IsType(ENTITY_CHARACTER)) {
			...
			if (found == NULL || vid != found->GetVID()) {
				SPDLOG_ERROR("<Factor> Invalid character {}", get_pointer(character));
				return;
			}
			...
			if (found == NULL || vid != found->GetVID()) {
				SPDLOG_ERROR("<Factor> Invalid item {}", get_pointer(item));
				return;
			}
			...
			if (found == NULL || vid != found->GetVID()) {
				SPDLOG_ERROR("<Factor> Invalid object {}", get_pointer(object));
				return;
			}
		} else {
			SPDLOG_ERROR("<Factor> Invalid entity type {}", get_pointer(entity));
			return;
		}
		*/
		...
		template <class _Func> void for_each_entity(_Func & func)
		{
			...
			for ( ; it != m_set_entity.end(); ++it) {
				...
				if (entity->GetSectree() != this) {
					SPDLOG_ERROR("<Factor> SECTREE-ENTITY relationship mismatch");
