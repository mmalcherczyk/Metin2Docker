...
namespace quest
{
	...
	int speedserver_get_wday (lua_State* L)
	{
		...
		if (wday < 0 || wday > 6)
		{
			SPDLOG_ERROR("wrong day");
			return 0;
		}

		SPDLOG_DEBUG("empire {} wday {}",empire, wday);
	...
	int speedserver_init_wday (lua_State* L)
	{
		...
		SPDLOG_DEBUG("init_wday {} {}",empire, wday);
	...
	int speedserver_get_holiday (lua_State* L)
	{
		...
		SPDLOG_DEBUG("empire {} date {} {} {}", empire, date.year, date.mon, date.day);
	...
	int speedserver_set_holiday (lua_State* L)
	{
		...
		SPDLOG_DEBUG("h {} m {} e {}", end_hour, end_minite, exp_percent);
	...
	int speedserver_get_current_exp_priv (lua_State* L)
	{
		...
		SPDLOG_DEBUG("empire : {} is_change : {}",empire, is_change);
