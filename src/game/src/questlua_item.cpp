...
namespace quest
{
	...
	int item_remove(lua_State* L)
	{
		...
		if (item != NULL) {
			if (q.GetCurrentCharacterPtr() == item->GetOwner()) {
				...
				quest_err("Tried to remove invalid item %p", get_pointer(item));
			}
			q.ClearCurrentItem();
		}
	...
	int item_has_flag(lua_State* L)
	{
		...
		if (!lua_isnumber(L, 1))
		{
			quest_err("flag is not a number.");
			lua_pushboolean(L, 0);
			return 1;
		}
	...
	int item_get_value(lua_State* L)
	{
		...
		if (!lua_isnumber(L, 1))
		{
			quest_err("index is not a number");
			lua_pushnumber(L, 0);
			return 1;
		}
		...
		{
			quest_err("index(%d) is out of range (0..%d)", index, ITEM_VALUES_MAX_NUM);
			lua_pushnumber(L, 0);
		}
		else
	...
	int item_next_refine_vnum(lua_State* L)
	{
		...
		{
			quest_err("Cannot find item table of vnum %u", vnum);
			lua_pushnumber(L, 0);
		}
		return 1;
	}
