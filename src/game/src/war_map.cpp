...
CWarMap::~CWarMap()
{
	...
	SPDLOG_DEBUG("WarMap::~WarMap : map index {}", GetMapIndex());
	...
	while (it != m_set_pkChr.end())
	{
		...
		if (ch->GetDesc())
		{
			SPDLOG_DEBUG("WarMap::~WarMap : disconnecting {}", ch->GetName());
...
void CWarMap::IncMember(LPCHARACTER ch)
{
	...
	if (isWarMember)
	{
		...
		SPDLOG_DEBUG("WarMap +m {}(cur:{}, acc:{}) vs {}(cur:{}, acc:{})",
...
void CWarMap::Timeout()
{
	...
	else
	{
		...
		if (iWinnerIdx == -1)
		{
			...
			SPDLOG_ERROR("WarMap: member count is not zero, guild1 {} {} guild2 {} {}, winner {}",
	...
	SPDLOG_DEBUG("WarMap: Timeout {} {} winner {} loser {} reward {} map {}",
...
bool CWarMap::CheckScore()
{
	...
	SPDLOG_DEBUG("WarMap::CheckScore end score {} guild1 {} score guild2 {} {} score {} winner {} reward {}",
			iEndScore,
			m_TeamData[0].dwID,
			m_TeamData[0].iScore,
			m_TeamData[1].dwID,
...
{
	SPDLOG_DEBUG("WarMap::SetEnded {}", m_kMapInfo.lMapIndex);
...
void CWarMap::OnKill(LPCHARACTER killer, LPCHARACTER ch)
{
	...
	SPDLOG_DEBUG("WarMap::OnKill {} {}", dwKillerGuild, dwDeadGuild);
	...
	switch (m_kMapInfo.bType)
	{
		...
		default:
			SPDLOG_ERROR("unknown war map type {} index {}", m_kMapInfo.bType, m_kMapInfo.lMapIndex);
			break;
	}
}
...
void CWarMap::AddFlagBase(BYTE bIdx, DWORD x, DWORD y)
{
	...
	SPDLOG_DEBUG("WarMap::AddFlagBase {} {} id {}", bIdx, (void*) get_pointer(r.pkChrFlagBase), r.dwID);
...
void CWarMap::AddFlag(BYTE bIdx, DWORD x, DWORD y)
{
	...
	SPDLOG_DEBUG("WarMap::AddFlag {} {} id {}", bIdx, (void*) get_pointer(r.pkChrFlag), r.dwID);
...
void CWarMap::RemoveFlag(BYTE bIdx)
{
	...
	SPDLOG_DEBUG("WarMap::RemoveFlag {} {}", bIdx, (void*) get_pointer(r.pkChrFlag));
...
bool CWarMapManager::GetStartPosition(int lMapIndex, BYTE bIdx, PIXEL_POSITION & pos)
{
	...
	if (!pi)
	{
		SPDLOG_ERROR("GetStartPosition FAILED [{}] WarMapInfoSize({})", lMapIndex, m_map_kWarMapInfo.size());
		...
		for (it	= m_map_kWarMapInfo.begin(); it != m_map_kWarMapInfo.end(); ++it)
		{
			...
			SPDLOG_ERROR("WarMap[{}]=Pos({}, {})", it->first, cur.x, cur.y);
		}
		return false;
	}
...
int CWarMapManager::CreateWarMap(const TGuildWarInfo& guildWarInfo, DWORD dwGuildID1, DWORD dwGuildID2)
{
	...
	if (!pkInfo)
	{
		SPDLOG_ERROR("GuildWar.CreateWarMap.NOT_FOUND_MAPINFO[{}]", guildWarInfo.lMapIndex);
		return 0;
	}
...
void CWarMapManager::DestroyWarMap(CWarMap* pMap)
{
	...
	SPDLOG_DEBUG("WarMap::DestroyWarMap : {}", mapIdx);
