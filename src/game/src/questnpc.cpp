...
// questpc.h: PC::typedef map<unsigned int, QuestState> QuestInfo;
// typedef 
...
namespace quest
{
		...
		while (itEventName != CQuestManager::instance().m_mapEventName.end())
		{
			...
			{	
				...
				while ((pde = readdir(pdir)))
				{
                    ...
                    SPDLOG_TRACE("QUEST reading {}", pde->d_name);
	...
	void NPC::LoadStateScript(int event_index, const char* filename, const char* script_name)
	{
        ...
        SPDLOG_TRACE("QUEST loading {} : {} [STATE] {}",
		...
		else
		{
			...
			if (i == s.npos)
			{
				SPDLOG_ERROR("invalid QUEST STATE index [{}] [{}]",filename, script_name);
				return;
			}
			...
			if (i != s.npos)
			{
				SPDLOG_ERROR("invalid QUEST STATE name [{}] [{}]",filename, script_name);
				return;
			}
	...
	{
		SPDLOG_DEBUG("OnTarget begin {} verb {} qi {}", c_pszTargetName, c_pszVerb, dwQuestIndex);
		...
		if (it == r.end())
		{
			SPDLOG_DEBUG("no target event, state {}", iState);
			return false;
		}
		...
		while (it_vec != it->second.end())
		{
			...
			SPDLOG_DEBUG("OnTarget compare {} {}", c_pszArg, argScript.arg.length());
			...
			if (argScript.when_condition.size() > 0)
				SPDLOG_DEBUG("OnTarget when {} size {}", &argScript.when_condition[0], argScript.when_condition.size());
	
			if (argScript.when_condition.size() != 0 && !IsScriptTrue(&argScript.when_condition[0], argScript.when_condition.size()))
				continue;

			SPDLOG_DEBUG("OnTarget execute qi {} st {} code {}", dwQuestIndex, iState, (const char *) argScript.script.GetCode());
