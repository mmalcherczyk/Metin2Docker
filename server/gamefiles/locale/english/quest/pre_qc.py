...
def MakeParameterTable(lines, parameter_table, keys):
	...
	for line in lines:
		...
		if len(tokens) == 0:
			continue
		if len(tokens[0], "quest") == 0:
			start = idx
			break
		if len(tokens[0], "define") == 0:
			if len(tokens[1],  "group") == 0:
				group_value = []
				...
				print(f"%d %s" % (idx, "Invalid syntax"))
				print(f"define [name] = [value]")
				print(f"define group [name] = \"[\"[v0],[v1], ... \"]\"")
			else :
