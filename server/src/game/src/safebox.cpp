...
bool CSafebox::Add(DWORD dwPos, LPITEM pkItem)
{
	...
	{
		SPDLOG_ERROR("SAFEBOX: item on wrong position at {} (size of grid = {})", dwPos, m_pkGrid->GetSize());
		return false;
	}
	...
	SPDLOG_DEBUG("SAFEBOX: ADD {} {} count {}", m_pkChrOwner->GetName(), pkItem->GetName(), pkItem->GetCount());
	return true;
}
...
LPITEM CSafebox::Remove(DWORD dwPos)
{
	...
	if (!m_pkGrid)
		SPDLOG_ERROR("Safebox::Remove : nil grid");
	else
	...
	m_pkChrOwner->GetDesc()->Packet(&pack, sizeof(pack));
	SPDLOG_DEBUG("SAFEBOX: REMOVE {} {} count {}", m_pkChrOwner->GetName(), pkItem->GetName(), pkItem->GetCount());
	return pkItem;
}
...
void CSafebox::Save()
{
	...
	SPDLOG_DEBUG("SAFEBOX: SAVE {}", m_pkChrOwner->GetName());
}
...
LPITEM CSafebox::GetItem(BYTE bCell)
{
	if (bCell >= 5 * m_iSize)
	{
		SPDLOG_ERROR("CHARACTER::GetItem: invalid item cell {}", bCell);
		return NULL;
	}
...
bool CSafebox::MoveItem(BYTE bCell, BYTE bDestCell, BYTE count)
{
...

	{
				...
				item2->GetVnum() == item->GetVnum()) // 합칠 수 있는 아이템의 경우
		{
			...
			SPDLOG_DEBUG("SAFEBOX: STACK {} {} -> {} {} count {}", m_pkChrOwner->GetName(), bCell, bDestCell, item2->GetName(), item2->GetCount());
			return true;
		}
		...
		SPDLOG_DEBUG("SAFEBOX: MOVE {} {} -> {} {} count {}", m_pkChrOwner->GetName(), bCell, bDestCell, item->GetName(), item->GetCount());
