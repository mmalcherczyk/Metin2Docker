...
namespace quest
{
	...
	int marriage_remove(lua_State* L)
	{
		...
		if (!pMarriage)
		{
			quest_err("pid[%d:%s] is not exist couple", ch->GetPlayerID(), ch->GetName());
			return 0;
		}
	...
	int marriage_join_wedding(lua_State* L)
	{
		...
		{
			quest_err("invalid player id for wedding map");
			return 0;
		}
		...
		{
			quest_err("not married %u %u", pid1, pid2);
			return 0;
		}
		//PREVENT_HACK
		if ( ch->IsHack() )
	...
	int marriage_wedding_dark(lua_State* L)
	{
		...
		{
			quest_err("invalid argument 1 : must be boolean");
			return 0;
		}
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		marriage::TMarriage* pMarriage = marriage::CManager::instance().Get(ch->GetPlayerID());
	...
	int marriage_wedding_client_command(lua_State* L)
	{
		if (!lua_isstring(L, 1))
		{
			quest_err("invalid argument 1 : must be string");
			return 0;
		}
	...
	int marriage_wedding_music(lua_State* L)
	{
		...
		if (!lua_isstring(L, 2))
		{
			quest_err("invalid argument 2 : must be string");
			return 0;
		}
	...
	int marriage_get_married_time(lua_State* L)
	{
		...
		if (!pMarriage)
		{
			quest_err("trying to get time for not married character");
			lua_pushnumber(L, 0);
			return 1;
		}
