...
namespace quest 
{
	...
	int pc_set_warp_location_local(lua_State * L)
	{
		...
		if (!region)
		{
			quest_err("invalid map index %d", lMapIndex);
			return 0;
		}
		...
		{
			quest_err("x coordinate overflow max: %d input: %d", region->ex - region->sx, x);
			return 0;
		}

		if (y > region->ey - region->sy)
		{
			quest_err("y coordinate overflow max: %d input: %d", region->ey - region->sy, y);
			return 0;
		}
	...
	int pc_warp_local(lua_State * L)
	{
		if (!lua_isnumber(L, 1)) 
		{
			quest_err("no map index argument");
			return 0;
		}

		if (!lua_isnumber(L, 2) || !lua_isnumber(L, 3))
		{
			quest_err("no coodinate argument");
			return 0;
		}
	...
	int pc_give_gold(lua_State* L)
	{
		...
		if (!lua_isnumber(L, 1))
		{
			quest_err("QUEST : wrong argument");
			return 0;
		}
		...
		if (iAmount <= 0)
		{
			quest_err("QUEST : gold amount less then zero");
			return 0;
		}
	...
	int pc_give_item(lua_State* L)
	{
		...
		{
			quest_err("QUEST Make item call error : wrong item name : %s", lua_tostring(L,1));
			return 0;
		}
		...
		if (lua_isnumber(L, 3) && lua_tonumber(L, 3) > 0)
		{
			...
			if (icount <= 0) 
			{
				quest_err("QUEST Make item call error : wrong item count : %g", lua_tonumber(L, 2));
				return 0;
			}
		}
	...
	int pc_give_or_drop_item(lua_State* L)
	{
		...
		{
			quest_err("QUEST Make item call error : wrong argument");
			lua_pushnumber (L, 0);
			return 1;
		}
		...
		SPDLOG_DEBUG("QUEST [REWARD] item {} to {}", lua_tostring(L, 1), ch->GetName());
	...
	int pc_count_item(lua_State* L)
	{
		...
		else if (lua_isstring(L, -1))
		{
			...
			{
				quest_err("QUEST count_item call error : wrong item name : %s", lua_tostring(L,1));
				lua_pushnumber(L, 0);
			}
			else
			{
	...
	int pc_remove_item(lua_State* L)
	{
		if (lua_gettop(L) == 1)
		{
			...
			else if (lua_isstring(L,1))
			{
				if (!ITEM_MANAGER::instance().GetVnum(lua_tostring(L,1), item_vnum))
				{
					quest_err("QUEST remove_item call error : wrong item name : %s", lua_tostring(L,1));
					return 0;
				}
			}
			else
			{
				quest_err("QUEST remove_item wrong argument");
				return 0;
			}

			SPDLOG_DEBUG("QUEST remove a item vnum {} of {}[{}]", item_vnum, CQuestManager::instance().GetCurrentCharacterPtr()->GetName(), CQuestManager::instance().GetCurrentCharacterPtr()->GetPlayerID());
		...
		else if (lua_gettop(L) == 2)
		{
			...
			SPDLOG_DEBUG("QUEST remove items(vnum {}) count {} of {}[{}]",
					item_vnum,
					item_count,
	...
	int pc_set_level(lua_State * L)  
	{
		...
		else
		{
			...
			SPDLOG_DEBUG("QUEST [LEVEL] {} jumpint to level {}", ch->GetName(), (int)rint(lua_tonumber(L,1)));
	...
	{		
		if (!lua_isnumber(L, 1))
		{
			quest_err("QUEST wrong set flag");
			return 0;
		}
	...
	int pc_change_money(lua_State * L)
	{
		...
		if (gold + ch->GetGold() < 0)
			quest_err("QUEST wrong ChangeGold %d (now %d)", gold, ch->GetGold());
		else
		{
	...
	int pc_get_another_quest_flag(lua_State* L)
	{
		...
		{
			quest_err("QUEST wrong get flag");
			return 0;
		}
		else
		{
	...
	int pc_get_quest_flag(lua_State* L)
	{
		...
		else
		{
            ...
            SPDLOG_TRACE("GetQF ( {} . {} )", pPC->GetCurrentQuestName().c_str(), sz );
		}
		return 1;
	}
	...
	int pc_give_exp2(lua_State* L)
	{
		...
		SPDLOG_DEBUG("QUEST [REWARD] {} give exp2 {}", ch->GetName(), (int)rint(lua_tonumber(L,1)));
	...
	int pc_give_exp(lua_State* L)
	{
		...
		SPDLOG_DEBUG("QUEST [REWARD] {} give exp {} {}", ch->GetName(), lua_tostring(L,1), (int)rint(lua_tonumber(L,2)));
	...
	int pc_give_exp_perc(lua_State* L)
	{
		...
		SPDLOG_DEBUG("QUEST [REWARD] {} give exp {} lev {} percent {}%", ch->GetName(), lua_tostring(L, 1), lev, proc);
	...
	{
		if (!lua_isnumber(L, 1))
			quest_err("QUEST wrong skillgroup number");
		else
		{
			CQuestManager & q = CQuestManager::Instance();
			LPCHARACTER ch = q.GetCurrentCharacterPtr();
	...
	int pc_have_pos_scroll(lua_State* L)
	{
		...
		if (!lua_isnumber(L,1) || !lua_isnumber(L,2))
		{
			quest_err("invalid x y position");
			lua_pushboolean(L, 0);
			return 1;
		}
	...
	int pc_get_equip_refine_level(lua_State* L)
	{
		...
		{
			quest_err("invalid wear position %d", cell);
			lua_pushnumber(L, 0);
			return 1;
		}
	...
	int pc_give_lotto(lua_State* L)
	{
		...
		SPDLOG_DEBUG("TRY GIVE LOTTO TO pid {}", ch->GetPlayerID());
	...
	int pc_clear_one_skill(lua_State* L)
	{
		...
		SPDLOG_DEBUG("{} skill clear", vnum);

		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if ( ch == NULL )
		{
			SPDLOG_ERROR("skill clear fail");
			lua_pushnumber(L, 0);
			return 1;
		}
...
teleport_area:
	...
	int pc_give_polymorph_book(lua_State* L)
	{
		...
		{
			quest_err("Wrong Quest Function Arguments: pc_give_polymorph_book");
			return 0;
		}
	...
	int pc_get_premium_remain_sec(lua_State* L)
	{
		...
		if (!lua_isnumber(L, 1)) 
		{
			quest_err("wrong premium index (is not number)");
			return 0;
		}
		...
		switch (premium_type)
		{
			...
			default:
				quest_err("wrong premium index %d", premium_type);
				return 0;
		}
	...
	int pc_give_award(lua_State* L)
	{
		...
		{
			quest_err("QUEST give award call error : wrong argument");
			lua_pushnumber (L, 0);
			return 1;
		}
        ...
        SPDLOG_DEBUG("QUEST [award] item {} to login {}", dwVnum, ch->GetDesc()->GetAccountTable().login);
