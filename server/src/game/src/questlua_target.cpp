...
namespace quest 
{
	...
	int target_pos(lua_State* L)
	{
		...
		{
			SPDLOG_ERROR("invalid argument, name: {}, quest_index {}", ch->GetName(), iQuestIndex);
			return 0;
		}
		...
		{
			SPDLOG_ERROR("cannot find base position in this map {}", ch->GetMapIndex());
			return 0;
		}
	...
	int target_id(lua_State* L)
	{
		...
		if (!lua_isstring(L, 1))
		{
			SPDLOG_ERROR("invalid argument, name: {}, quest_index {}", ch->GetName(), dwQuestIndex);
			lua_pushnumber(L, 0);
			return 1;
		}
