...
SPDLOG_DEBUG("SendTargetUpdatePacket {} {}x{}", iID, x, y);
}
...
// <Factor> Raplaced direct pointer reference with key searching.
	...
	{
		SPDLOG_DEBUG("target_event: event canceled");
		return 0;
	}
...
void CTargetManager::CreateTarget(DWORD dwPID,
		...
		int iSendFlag)
{
	SPDLOG_DEBUG("CreateTarget : target pid {} quest {} name {} arg {} {} {}",
	...
	if (!pkChr)
	{
		SPDLOG_ERROR("Cannot find character ptr by PID {}", dwPID);
		return;
	}
	...
	if (it != m_map_kListEvent.end())
	{
		...
		while (it2 != it->second.end())
		{
			...
			{
				SPDLOG_DEBUG("CreateTarget : same target will be replaced");
