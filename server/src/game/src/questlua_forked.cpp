...
namespace quest
{
	...
	int forked_sungzi_mapindex (lua_State *L )
	{
        ...
        SPDLOG_TRACE("forked_sungzi_map_index_by_empire {}", GetSungziMapIndex() );
		return 1;
	}
	...
	int forked_get_pass_path_my_empire( lua_State *L )
	{
		...
		SPDLOG_DEBUG("[PASS_PATH] Empire {} Path  {}", ch->GetEmpire(), GetPassMapPath( ch->GetEmpire() ) );
		return 1;
	}
	...
	int forked_get_pass_path_by_empire( lua_State *L )
	{
		...
		SPDLOG_DEBUG("[PASS_PATH] Empire {} Path  {}", iEmpire, GetPassMapPath( iEmpire ) );
		return 1;
	}
