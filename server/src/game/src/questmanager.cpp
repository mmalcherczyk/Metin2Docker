...
namespace quest
{
	...
	bool CQuestManager::Initialize()
	{
...

		{
			...
			if (!inf.is_open())
				...
				SPDLOG_DEBUG("QUEST can open 'questnpc.txt' ({})", g_stQuestDir.c_str() );
			...
			while (1)
			{
				...
				if (ri < li) 
				{
					SPDLOG_ERROR("QUEST questnpc.txt:{}:npc name error",line);
					continue;
				}
                ...
                SPDLOG_TRACE("QUEST reading script of {}({})", s.c_str(), vnum);
	...
	void CQuestManager::Confirm(unsigned int pc, EQuestConfirmType confirm, unsigned int pc2)
	{
		...
		if (!pPC->IsRunning())
		{
			SPDLOG_ERROR("no quest running for pc, cannot process input : {}", pc);
			return;
		}

		if (pPC->GetRunningQuestState()->suspend_state != SUSPEND_STATE_CONFIRM)
		{
			SPDLOG_ERROR("not wait for a confirm : {} {}", pc, pPC->GetRunningQuestState()->suspend_state);
			return;
		}
	...
	void CQuestManager::Input(unsigned int pc, const char* msg)
	{
		...
		{
			SPDLOG_ERROR("not wait for a input : {} {}", pc, pPC->GetRunningQuestState()->suspend_state);
			return;
		}
	...
	void CQuestManager::Select(unsigned int pc, unsigned int selection)
	{
		...
		{
			SPDLOG_ERROR("wrong QUEST_SELECT request! : {}",pc);
		}
	}
	...
	void CQuestManager::Resume(unsigned int pc)
	{
		...
		else
		{
			...
			SPDLOG_ERROR("wrong QUEST_WAIT request! : {}",pc);
		}
	}
	...
	void CQuestManager::Kill(unsigned int pc, unsigned int npc)
	{
		...
		SPDLOG_DEBUG("CQuestManager::Kill QUEST_KILL_EVENT (pc={}, npc={})", pc, npc);
	...
	bool CQuestManager::ServerTimer(unsigned int npc, unsigned int arg)
	{
		...
		SPDLOG_DEBUG("XXX ServerTimer Call NPC {}", (void*) GetPCForce(0));
	...
	bool CQuestManager::Timer(unsigned int pc, unsigned int npc)
	{
		...
		else
		{
			...
			SPDLOG_ERROR("QUEST TIMER_EVENT no such pc id : {}", pc);
			return false;
		}
	...
	void CQuestManager::LevelUp(unsigned int pc)
	{
		...
		{
			SPDLOG_ERROR("QUEST LEVELUP_EVENT no such pc id : {}", pc);
		}
	}
	...
	void CQuestManager::QuestInfo(unsigned int pc, unsigned int quest_index)
	{
		...
		{
			//cout << "no such pc id : " << pc;
			SPDLOG_ERROR("QUEST INFO_EVENT no such pc id : {}", pc);
		}
	}
	...
	void CQuestManager::QuestButton(unsigned int pc, unsigned int quest_index)
	{
		...
		{
			//cout << "no such pc id : " << pc;
			SPDLOG_ERROR("QUEST CLICK_EVENT no such pc id : {}", pc);
		}
	}
	...
	bool CQuestManager::TakeItem(unsigned int pc, unsigned int npc, LPITEM item)
	{
		...
		{
			//cout << "no such pc id : " << pc;
			SPDLOG_ERROR("QUEST USE_ITEM_EVENT no such pc id : {}", pc);
			return false;
		}
	}
	...
	{
        SPDLOG_TRACE("questmanager::UseItem Start : itemVnum : {} PC : {}", item->GetOriginalVnum(), pc);
		PC* pPC;
		if ((pPC = GetPC(pc)))
		{
			if (!CheckQuestLoaded(pPC))
			...
			if (test_server)
			{
				SPDLOG_DEBUG("Quest UseItem Start : itemVnum : {} PC : {}", item->GetOriginalVnum(), pc);
				itertype(m_mapNPC) it = m_mapNPC.begin();
				itertype(m_mapNPC) end = m_mapNPC.end();
				for( ; it != end ; ++it)
				{
					SPDLOG_DEBUG("Quest UseItem : vnum : {} item Vnum : {}", it->first, item->GetOriginalVnum());
				}
			}
			SPDLOG_TRACE("questmanager:useItem: mapNPCVnum : {}", m_mapNPC[item->GetVnum()].GetVnum());
			*/
	...
	bool CQuestManager::Click(unsigned int pc, LPCHARACTER pkChrTarget)
	{
		...
		if (pPC)
		{
            ...
            SPDLOG_TRACE("CQuestManager::Click(pid={}, npc_name={}) - target_info({})", pc, pkChrTarget->GetName(), (void*) pInfo);
			...
			if (pkChrTarget->IsNPC())
			{
				...
				if (it == m_mapNPC.end())
				{
					SPDLOG_ERROR("CQuestManager::Click(pid={}, target_npc_name={}) - NOT EXIST NPC RACE VNUM[{}]",
							pc, 
							pkChrTarget->GetName(), 
							dwCurrentNPCRace);
					return false;
	...
	void CQuestManager::LoadStartQuest(const string& quest_name, unsigned int idx)
	{
		for (itertype(g_setQuestObjectDir) it = g_setQuestObjectDir.begin(); it != g_setQuestObjectDir.end(); ++it)
		{
			...
			if (inf.is_open())
			{
				SPDLOG_DEBUG("QUEST loading begin condition for {}", quest_name.c_str());
	...
	void CQuestManager::SendScript()
	{
        ...
        SPDLOG_TRACE("m_strScript {} size {}", m_strScript.c_str(), buf.size());
	...
	const char* CQuestManager::GetQuestStateName(const string& quest_name, const int state_index)
	{
		...
		if (lua_isnil(L,-1))
		{
			SPDLOG_ERROR("QUEST wrong quest state file {}.{}", quest_name.c_str(), state_index);
			lua_settop(L,x);
			return "";
		}
	...
	int CQuestManager::GetQuestStateIndex(const string& quest_name, const string& state_name)
	{
		...
		if (lua_isnil(L,-1))
		{
			SPDLOG_ERROR("QUEST wrong quest state file {}.{}",quest_name.c_str(),state_name.c_str()  );
			lua_settop(L,x);
			return 0;
		}
		...
		lua_settop(L, x);
        SPDLOG_TRACE("[QUESTMANAGER] GetQuestStateIndex x({}) v({}) {} {}", v,x, quest_name.c_str(), state_name.c_str() );
		return v;
	}
	...
	void CQuestManager::RegisterQuest(const string & stQuestName, unsigned int idx)
	{
		...
		SPDLOG_DEBUG("QUEST: Register {:4} {}", idx, stQuestName.c_str());
	}
	...
	const string & CQuestManager::GetQuestNameByIndex(unsigned int idx)
	{
		...
		{
			SPDLOG_ERROR("cannot find quest name by index {}", idx);
	...
	void CQuestManager::SetEventFlag(const string& name, int value)
	{
        ...
        SPDLOG_TRACE("QUEST eventflag {} {} prev_value {}", name, value, m_mapEventFlag[name]);
			...
			if (value && !prev_value)
			{
				...
				while (pPosition->lMapIndex)
				{
					if (map_allow_find(pPosition->lMapIndex))
					{
						...
						{
							SPDLOG_ERROR("cannot get map base position {}", pPosition->lMapIndex);
							++pPosition;
							continue;
						}
	...
	void CQuestManager::WriteRunningStateToSyserr()
	{
		...
		SPDLOG_ERROR("LUA_ERROR: quest {}.{} {}", GetCurrentQuestName().c_str(), state_name, event_index_name.c_str() );
	...
	{
        SPDLOG_TRACE("questmanager::PickupItem Start : itemVnum : {} PC : {}", item->GetOriginalVnum(), pc);
		PC* pPC;
		if ((pPC = GetPC(pc)))
		{
			if (!CheckQuestLoaded(pPC))
		...
		{
			SPDLOG_ERROR("QUEST PICK_ITEM_EVENT no such pc id : {}", pc);
			return false;
		}
	}
	...
	void CQuestManager::EndOtherPCBlock()
	{
		...
		{
			SPDLOG_ERROR("m_vecPCStack is alread empty. CurrentQuest{Name({}), State({})}", GetCurrentQuestName().c_str(), GetCurrentState()->_title.c_str());
			return;
		}
