...
namespace quest
{
	...
	{
        SPDLOG_TRACE("QUEST Setting flag {} {}", name.c_str(),value);
	...
	int PC::GetFlag(const string & name)
	{
		...
		if (it != m_FlagMap.end())
		{
			SPDLOG_DEBUG("QUEST getting flag {} {}", name.c_str(),it->second);
			return it->second;
		}
		return 0;
	}
	...
	void PC::AddQuestStateChange(const string& quest_name, int prev_state, int next_state)
	{
		...
		SPDLOG_DEBUG("QUEST reserve Quest State Change quest {}[{}] from {} to {}", quest_name.c_str(), dwQuestIndex, prev_state, next_state);
	...
	void PC::RemoveTimerNotCancel(const string & name)
	{
		...
		SPDLOG_DEBUG("QUEST timer map size {} by RemoveTimerNotCancel", m_TimerMap.size());
	}
	...
	void PC::RemoveTimer(const string & name)
	{
		...
		SPDLOG_DEBUG("QUEST timer map size {} by RemoveTimer", m_TimerMap.size());
	}

	void PC::ClearTimer()
	{
		SPDLOG_DEBUG("QUEST clear timer {}", m_TimerMap.size());
	...
	void PC::SendQuestInfoPakcet()
	{
		...
		if (m_iSendToClient & QUEST_SEND_ISBEGIN)
		{
			...
			SPDLOG_DEBUG("QUEST BeginFlag {}", (int)temp);
		}
		if (m_iSendToClient & QUEST_SEND_TITLE)
		{
			...
			SPDLOG_DEBUG("QUEST Title {}", m_RunningQuestState->_title.c_str());
		}
		if (m_iSendToClient & QUEST_SEND_CLOCK_NAME)
		{
			...
			SPDLOG_DEBUG("QUEST Clock Name {}", m_RunningQuestState->_clock_name.c_str());
		}
		if (m_iSendToClient & QUEST_SEND_CLOCK_VALUE)
		{
			...
			SPDLOG_DEBUG("QUEST Clock Value {}", m_RunningQuestState->_clock_value);
		}
		if (m_iSendToClient & QUEST_SEND_COUNTER_NAME)
		{
			...
			SPDLOG_DEBUG("QUEST Counter Name {}", m_RunningQuestState->_counter_name.c_str());
		}
		...
		if (m_iSendToClient & QUEST_SEND_ICON_FILE)
		{
			...
			SPDLOG_DEBUG("QUEST Icon File {}", m_RunningQuestState->_icon_file.c_str());
		}
	...
	void PC::EndRunning()
	{
		// unlocked locked npc
		{
			...
			if (npc && !npc->IsPC())
			{
				...
				{
					npc->SetQuestNPCID(0);
					SPDLOG_WARN("QUEST NPC lock isn't unlocked : pid {}", ch->GetPlayerID());
		...
		if (m_iSendToClient)
		{
			SPDLOG_DEBUG("QUEST end running {}", m_iSendToClient);
			SendQuestInfoPakcet();
		}

		if (m_RunningQuestState == NULL) {
			SPDLOG_ERROR("Entered PC::EndRunning() with invalid running quest state");
			return;
		}
	...
	void PC::DoQuestStateChange()
	{
		...
		for (DWORD i=0; i<vecQuestStateChange.size(); ++i)
		{
			...
			SPDLOG_DEBUG("QUEST change reserved Quest State Change quest {} from {} to {} ({} {})", dwQuestIdx, rInfo.prev_state, rInfo.next_state, it->second.st, rInfo.prev_state );
	...
	void PC::Save()
	{
		...
		while (it != m_FlagSaveMap.end())
		{
			...
			SPDLOG_DEBUG("QUEST Save Flag {}, {} {} ({})", stName.c_str(), stState.c_str(), lValue, i);
	...
	{
		SPDLOG_DEBUG("QUEST GiveItem {} {} {}", label.c_str(),dwVnum,count);
	...
	{
		SPDLOG_DEBUG("QUEST GiveExp {} {}", label.c_str(),exp);
		...
		for (vector<RewardData>::iterator it = m_vRewardData.begin(); it != m_vRewardData.end(); ++it)
		{
			switch (it->type)
			{
				case RewardData::REWARD_TYPE_EXP:
					SPDLOG_DEBUG("EXP cur {} add {} next {}",ch->GetExp(), it->value1, ch->GetNextExp());
